        ...
        switch (algorithm) {
            ...
            case Ramanujan: {
                ...
                */
                for (long i = 0; i < iterations; i++) {
                    BigDecimal bigI = BigDecimal.valueOf(i);
                    BigDecimal first = new BigDecimal(4).multiply(bigI);
                    first = utilities.factorial(first);
                    ...
                    first = bigI;
                    first = utilities.factorial(first);
                    first = utilities.pow(first, BigDecimal.valueOf(4));
                    second = new BigDecimal(396);
                    second = utilities.pow(second, BigDecimal.valueOf(4).multiply(bigI));
            ...
            case Gauss_Legendre: {
                ...
                BigDecimal an = BigDecimal.ONE,
                        ...
                        tn = BigDecimal.ONE.divide(BigDecimal.valueOf(4), mathContext),
                        pn = an;
                for (long i = 0; i < iterations; i++) {
                    ...
                    tn1 = utilities.pow(tn1, BigDecimal.TWO);
                    tn1 = tn1.multiply(pn);
                    tn1 = tn.subtract(tn1);
                ...
                result = an.add(bn);
                result = utilities.pow(result,BigDecimal.TWO);
                result = result.divide(tn.multiply(BigDecimal.valueOf(4)), mathContext);
                return result;
            }
            ...
            case Madhava_Leibniz: {
                ...
                result = result.multiply(new BigDecimal(4),mathContext);
                return result;
            }
            case Euler: {
                ...
                for (long i = 1; i < iterations; i++) {
                    BigDecimal bigI = BigDecimal.valueOf(i);
                    partialResult =  BigDecimal.ONE.divide(utilities.pow(bigI,BigDecimal.TWO),mathContext);
